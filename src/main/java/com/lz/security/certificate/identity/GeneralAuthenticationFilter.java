...
public class GeneralAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException, IOException, ServletException {
        ...
        switch (obtainParam(request, SPRING_SECURITY_FORM_LOGINTYPE_KEY)) {
            case "formLogin":
                String username = obtainParam(request, SPRING_SECURITY_FORM_USERNAME_KEY);
                String password = obtainParam(request, SPRING_SECURITY_FORM_PASSWORD_KEY);

                GeneralAuthenticationToken authRequest = new GeneralAuthenticationToken(
                        username, password);
    ...
    private String obtainParam(HttpServletRequest request, String paramKay) {
        if (request.getContentType() != null && (request.getContentType().contains(MediaType.APPLICATION_JSON_VALUE) || request.getContentType().contains(MediaType.APPLICATION_JSON_UTF8_VALUE))) {
            ...
            try {
                ...
                return loginData.get(paramKay) == null ? "" : loginData.get(paramKay).trim();
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
